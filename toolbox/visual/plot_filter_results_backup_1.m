...
%   plot_filter_results(t,x,x_true,x_lower,x_upper)
...
%   x_true      - (1×N double) true state variable time history
...
function plot_filter_results(t,x,x_true,x_lower,x_upper,opts)
    ...
    if shaded
        ...
        plot(t,x_lower,'k','handlevisibility','off');
    ...
    if include_legend
        ...
        % obtain avg. Mσ bound (ignore first 10% of data due to transience)
        ...
        if name == ""
            estimate_str = "state estimate ($\pm"+avg_bound_str+...
                "\;\mathrm{"+xunits+"}$)";
        else
            estimate_str = ""+name+" estimate ($"+avg_bound_str+...
                "$\mathrm{"+xunits+"}$)";
        end
        ...
        if name == ""
            ...
            covariance_str = ""+name+" estimate ($"+avg_bound_str+...
                "$\mathrm{"+xunits+"}$)";
        end
        ...
        % legend
        legend(estimate_str,true_str,'interpreter','latex','fontsize',...
